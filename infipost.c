...
int main(int argc, char **argv)
{
  ...
  unsigned int exp_len=2*strlen(argv[1])+1 ;

  char work_buff[exp_len];
  char postfx[exp_len];
  bzero(postfx, exp_len);
  strcpy(work_buff, argv[1]);
  
  int er_code=-1;
  if(er_code=(to_postfix(work_buff, postfx, exp_len)))
  {
    puts("erronous expression");
    printf("%s\n%d\n", postfx, er_code);
    return 1;
  }
  printf("%s\n", postfx);
  printf("%.3f\n", evaluate(postfx, exp_len));
  return 0;
}
